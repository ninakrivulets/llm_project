...
Наш ассистент не требует ввода персональных данных — пользователю достаточно задать вопрос о рецепте.
Мы можем хранить историю запросов для анализа качества работы, но без привязки к личности (только обезличенный user_id).
Все данные передаются по защищенному соединению (HTTPS). В LLM не отправляются персональные данные пользователя.
Если проект будет запущен публично, пользователю будет доступна политика конфиденциальности и уведомление о логировании запросов.

**Устранение вредоносного/нежелательного контента, фильтры:**
ассистент отвечает только на вопросы, связанные с рецептами и приготовлением;
в системном промпте зафиксирован запрет на медицинские, опасные или незаконные рекомендации;
при оффтопик-запросах бот корректно сообщает, что работает только в кулинарной области;
при необходимости применяется базовая фильтрация входных сообщений.

**Прозрачность**: уведомление о том, что это бот, логика сбора данных:
Пользователь всегда понимает, что общается с AI-ассистентом, а не с человеком.
Ответы формируются автоматически на основе базы рецептов (RAG-подход), поэтому возможны неточности.
Для улучшения качества предусмотрена возможность оставить обратную связь (например, лайк/дизлайк ответа).
Регулирования и нормативы (GDPR, внутренние политики):
В случае реального внедрения сервис должен соответствовать требованиям законодательства о персональных данных (например, GDPR или 152-ФЗ в РФ).
Предусматривается ограниченный срок хранения логов и возможность удаления пользовательской истории по запросу.

### 7.1. План внедрения и эксплуатации: Этапы проекта

Фаза 0: исследование/документирование. 
анализ кулинарных сайтов как источников данных;
определение архитектуры решения (RAG + LLM);
выбор языковой модели (например, Qwen или YandexGPT);
проектирование структуры базы рецептов и схемы индексирования;
подготовка и согласование Design Doc.

Фаза 1: MVP (минимально жизнеспособный бот).
осуществляется парсинг рецептов и формирование базы данных;
выполняется разбиение текстов на чанки и векторизация;
настраивается интеграция с LLM;
реализуется RAG-логика (retrieval + generation);
создается простой чат-интерфейс;
выполняется контейнеризация проекта (Docker).

Фаза 2: расширение функциональности, RAG-дополнения.
улучшение качества retrieval и chunking-стратегии;
добавление метаданных (тип кухни, время приготовления, диетические категории);
оптимизация промптов;
внедрение логирования и сбора пользовательского фидбэка;
реализация обработки исключительных сценариев (нет релевантного ответа, оффтопик).

Фаза 3: масштабирование, оптимизация.
оптимизация времени ответа системы;
кеширование популярных запросов;
автоматическое обновление базы рецептов;
разделение окружений (dev / test / prod);
подготовка к увеличению нагрузки.
Вехи, сроки, ответственные.
Каждый этап проекта завершается конкретным рабочим результатом, который можно продемонстрировать и протестировать. Это позволяет постепенно проверять гипотезы и при необходимости скорректировать архитектурные решения.
В проекте участвуют четыре человека, поэтому задачи распределяются по направлениям: работа с моделью и RAG-логикой, сбор и подготовка данных, backend и API, а также инфраструктура и контейнеризация. Такое распределение позволяет вести разработку параллельно и ускоряет реализацию.
Ориентировочный срок разработки MVP составляет 2–4 недели и зависит от объёма данных, сложности RAG-логики и выбранной языковой модели.

### 7.2. План внедрения и эксплуатации: Поддержка и эксплуатация
**Ответственность за поддержку**
Поддержка системы распределяется внутри проектной команды.
В рамках учебного проекта роли могут совмещаться.

**Мониторинг**: логирование ошибок, SLA, alerts, dashboards.
логирование пользовательских запросов и ответов модели;
фиксация ошибок и времени ответа;
базовый контроль доступности сервиса.

**Обновление знаний**: план синхронизации, процесс ревью.
База рецептов может обновляться по заранее определённому графику (например, периодический парсинг новых данных).
При добавлении новых рецептов выполняется:
предобработка текста,
векторизация,
обновление индекса.
Также предусматривается возможность ручной проверки и корректировки данных при необходимости.

**Метрики эксплуатации**: uptime, latency, cost per session.
Uptime — доступность сервиса;
Latency — среднее время ответа;
Cost per session — вычислительные затраты на один пользовательский запрос (при использовании внешней LLM).
